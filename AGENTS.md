...
It is an **npm**-managed **Turborepo** monorepo.
...
- `apps/backend` — TypeScript + Express API. Executed via **tsx** under Node.js (`npm run dev` / `npm start`).
...
- Package manager is **npm**. Use `npm install`, `npm run <script>`.
- Backend imports use `.js` extensions (NodeNext); tsx resolves them to `.ts` at runtime.
...
npm run dev                 # run all apps with hot reload (turbo)
...
```sh
npm run infra:up            # Postgres + MinIO only (for host-based dev)
npm run docker:up           # full stack in Docker
npm run docker:facefusion   # build + start the FaceFusion swap service (~5GB, needed for face swap)
npm run docker:reset        # stop + wipe volumes (DESTRUCTIVE)
```
...
2. `npm run build` (frontend `vite build` + backend `tsc --noEmit`).
3. For backend logic, smoke-test by booting `npm run --prefix apps/backend start`
