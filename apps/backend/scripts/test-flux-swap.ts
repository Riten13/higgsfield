 ...
 *   npm run --prefix apps/backend scripts/test-flux-swap.ts
 *
 *   # Real swap (spends a few cents): grab two images and write the result.
 *   npm run --prefix apps/backend scripts/test-flux-swap.ts <faceImg> <frameImg> [context]
 *
...
async function main() {
  ...
  if (!faceArg || !frameArg) {
    ...
    await metadataCheck();
    console.log(
      "\nTo run a real swap: npm run --prefix apps/backend scripts/test-flux-swap.ts <faceImg> <frameImg> [context]",
    );
    return;
  }
