...
This is a [Turborepo](https://turborepo.dev) monorepo managed with **npm**.
...
npm run docker:reset         # stop AND delete volumes (wipes DB + objects)
```
...
npm run db:generate
npm run db:push          # sync schema -> Postgres (use db:migrate once you add migrations)
...
| `npm run docker:reset`  | Stop the stack and delete volumes.           |
| `npm run docker:facefusion` | Build & start the FaceFusion face-swap service. |
| `npm run infra:up`      | Start only Postgres + MinIO.                 |
| `npm run dev`           | Run all apps locally with hot reload.        |
